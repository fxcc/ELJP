...
from HAN import HierAttNet, HAN_with_sent_label
import random
import numpy as np
import json
import math
...
def main():
    ...
    # model = HierAttNet(vocab_size=len(tokenizer.vocab), word_hidden_size=256, sent_hidden_size=256,
    #                    num_classes=len(set(prisons)), embed_size=256,
    #                    dropout=0.1, embedding=None, padding_idx=None)
    model = HAN_with_sent_label(vocab_size=len(tokenizer.vocab), word_hidden_size=256, sent_hidden_size=256,
                                num_classes=len(set(prisons)), embed_size=256, sentence_classes=2,
                                dropout=0.1, embedding=None, padding_idx=None)
    model.to(device)
    model.train()
    ...
    for epoch in range(max_epoch):
        ...
        for i, batch in enumerate(train_dataloader):
            ...
            #       prison.size())
            output, logits = model(input_ids)  # penalty cls
            loss1 = loss_fct(output, prison)
            loss2 = loss_fct(logits.view(-1, 2), annotations.view(-1))
            loss = loss1 + loss2
